...
from tensorflow.keras.utils import multi_gpu_model # for multiple gpus
...
def generate_data_3d(t_filenames, o_filenames, min_slc = 67, max_slc = 217, x_min = None, x_max = None, y_min = None, y_max = None):
    ...
    for i in range(len(t_filenames)):
        ...
        if x_min is not None and x_max is not None:
            if y_min is not None and y_max is not None:
                # x and y limits
                x_mri = x_mri[min_slc:max_slc, x_min:x_max, y_min:y_max]
                y_mri = y_mri[min_slc:max_slc, x_min:x_max, y_min:y_max]
            else:
                # only x limits
                x_mri = x_mri[min_slc:max_slc, x_min:x_max, :]
                y_mri = y_mri[min_slc:max_slc, x_min:x_max, :]
        elif y_min is not None and y_max is not None:
            # only y limits
            x_mri = x_mri[min_slc:max_slc, :, y_min:y_max]
            y_mri = y_mri[min_slc:max_slc, :, y_min:y_max]
        else:
            # no x and y limits
            x_mri = x_mri[min_slc:max_slc]
            y_mri = y_mri[min_slc:max_slc]
        # Normalize data
...
def small_unet_model(lr=1e-4, input_size=(256, 256, 176, 1), dropout_level=0.1):
    inputs = Input(input_size)
    ...
    up7 = conv_layer(256, 2)(UpSampling3D(size=(2, 2, 2))(drop4))
    merge7 = concatenate([conv3, up7], axis=4)
    drop7 = Dropout(dropout_level)(merge7)
    conv7 = conv_layer(256, 3)(drop7)
    conv7 = conv_layer(256, 3)(conv7)
...
lim = 5
X_train, y_train = generate_data_3d(X_filenames[:lim], y_filenames[:lim], min_slc = 73, max_slc = 217, x_min = 32, x_max = 256)
...
#     model = small_unet_model(input_size=(144, 224, 176, 1))
# model = multi_gpu_model(model, gpus=num_gpus)


# model = unet_model(input_size=(144, 224, 176, 1))
model = small_unet_model(input_size=(144, 224, 176, 1))
model.fit(X_train, y_train, batch_size=1, validation_split=0.2, epochs=3, use_multiprocessing=True, callbacks=callbacks_list, verbose=1)